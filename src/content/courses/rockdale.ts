...
export const ROCKDALE_MUNI: readonly HoleSpec[] = [
  ...
  },
  {
    ...
    // v6 (REVIEW-5: difficulty at the front leaked out the back, so the next
    // increment goes at the back, where the shadow lands on 7 and on
    // nothing). The corridor tightens two yards through the landing zone and
    // at the green — changes-4's own named next step. The pond, the finger,
    // the green: untouched.
    num: 6, par: 4, length: 325, name: 'Retention Pond',
    corridor: [{ at: 0, half: 24 }, { at: 210, half: 19 }, { at: 325, half: 17 }],
  ...
  },
  {
    ...
    // v6 (REVIEW-5): the last hole's shadow lands on nothing, so difficulty
    // here is the one free lunch on the course. The parking lot moves in six
    // yards (−48..−32): the pin-aimed Bomb still can't find it, the ripped
    // one can, so "a genuine hook" is finally a thing the cards can hit. The
    // green tightens 14 → 13 — still a birdie look, now it wants the good
    // swing. Green light, slightly narrower street.
    num: 8, par: 4, length: 320, name: 'Last Call',
    corridor: [{ at: 0, half: 26 }, { at: 200, half: 24 }, { at: 320, half: 22 }],
    greenRadius: 13, greenSide: 0,
    hazards: [
      { surface: 'ob', at: { down: 250, side: -40 }, rDown: 90, rSide: 8 },
    ],
