 ...
 *
 * Order (v6): fork, funding round, fork, fork (par 15, no par 5). REVIEW-5's
 * diagnostic showed the original four-fork front starving its own tees —
 * 1.5 focus at the 3rd against Rockdale's 3.0 — because a fork chain drains
 * itself: whoever pays at the 1st arrives poorer at the 2nd. So the 2nd is
...
export const SALT_FLATS: readonly HoleSpec[] = [
  {
    ...
    // v6 (REVIEW-5: 0.10, and harder — the belt worked, the price didn't).
    // The pan sat 23–37 off the line: the loose Bomb's edge is ±17.6 and the
    // ripped Bomb's ±26, so "into the pan's price" was a claim, not a fact —
    // nothing the player can hit could reach it. Moved to 15–27: the naked
    // Bomb (±13) still misses it, the loose one clips it, the ripped one
    // lives in it. Now the three ways across genuinely cost differently.
    ...
    hazards: [
      ...
      { surface: 'ob', at: { down: 285, side: 21 }, rDown: 60, rSide: 6 },
    ],
    note: 'The crust ends at two-seventy and the Bomb carries two-sixty-five. Buy the difference or lay up and pay in strokes.',
  },
  {
    // v6 — the funding round, per REVIEW-5 §1. The old 2nd was the course's
    // biggest spender (tee 2 arrived at 4.6 and tee 3 at 1.5), which meant
    // the forks at 3 and 4 were played by a man who could not afford either
    // branch. This hole is built to cost nothing: 365 sits ON the ladder —
    // Bomb finishes 277 and leaves 88 (Full Wedge cuts down), Stinger leaves
    // 110 (wedge plus the free Extra Club) — so every appetite reaches in
    // two without buying a technique, and the only focus spent here is spent
    // VOLUNTARILY, on a birdie putt. The waste bunkers sit 18+ off the line,
    ...
    greenRadius: 15, greenSide: 0,
    hazards: [
      { surface: 'bunker', at: { down: 350, side: -26 }, rDown: 12, rSide: 8 },
      { surface: 'bunker', at: { down: 350, side: 26 }, rDown: 12, rSide: 8 },
    ],
    note: 'No crust, no pan, nothing to carry but the water you brought. Drink now. The next three holes are the reason.',
  },
  {
