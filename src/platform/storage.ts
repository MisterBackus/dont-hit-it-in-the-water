...
export const SAVE_VERSION = 12 // v12: THE ENCOUNTER STAKES (ENCOUNTER-STAKES.md) — reducer.ts applyOutcome resolves encounter money as POINTS of the live Money List demand instead of four written dollar constants, so a v11 log that took a fine, signed an autograph or shook the sandbagger's hand replays with different money; the sandbagger's minWallet moves with his stake, so in a narrow early-wallet window a replay can also meet a different person. Draw counts are untouched everywhere — the same two 'events' rolls decide who shows, the same two settle a gamble — so it is the same run, paid differently: the v8 class of bump. (The night's other reducer-observable change, the sharpness ramp's new knee — SHARPNESS.md, every cone in a replayed log — rides this same version rather than claiming a second one.) // v11: THE FULL SCORECARD (FIELD-SPREAD.md SHIPPED) — the field finishes its 36-hole week at settle and the player's remainder is rolled (salts 10/11), places and cheques move to 36-hole space with the full real-tour tie split at every rank, so a v10 log replays as a different season: same shots, different money from the first settle. The frozen ledger keeps every verified row. // v10: the junk spread floor (JUNK-VERDICT.md SHIPPED) — buildCone imposes a 12-yard minimum scatter on rough/deep/bunker/trees, so every jungle shot in a v9 log resolves from a different cone and the run replays differently // v9: the shop-supply hybrid (SHOP-SUPPLY.md) — the season boost budget makes a seventh BUY illegal on replay, and the tiered stock draws from a new `shop` rng stream (weighted tiers, reroll within the week's draw), so a v8 log's shop offers replay differently // v8: split at the top — a tied WIN now pays the mean of the covered places (settle also writes the seasonRecord), so a v7 log replays with different money // v7: the weeks redesign — the draw is biased early, silent at majors and from event 10, and a sponsor expires after three events, so a v6 log replays as a different run // v6: encounters after the cut — a v5 log has no ENGAGE/WALK_ON and one made cut in three now stops at the fifth tee // v5: ten-course pool + new pins reshuffle every seed's schedule // v4: the schedule rotation — events draw courses from the pool, so a v3 log replays as a different run
